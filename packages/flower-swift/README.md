...
The UniFFI binding lives under `uniffi-generated/` in this directory and is
**committed**, regenerated by `scripts/gen-bindings.sh` and held to
`crates/flower-ffi` by CI's `bindings` job — so the package builds from a bare
checkout with no generator run. The consuming app force-loads the Rust staticlib
and rebuilds it on every Xcode build, so ordinary Rust edits need only ⌘R
(re-run `gen-bindings.sh` only when the FFI *API* changes).

The `Package.swift` at the **repo root** exposes the library products (take it
by version, `.package(url: "https://github.com/diaryx-org/flower.git", from:
"X.Y.Z")`, or as a local package, `.package(path: "…/flower")`):
