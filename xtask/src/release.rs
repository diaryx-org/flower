...
//!
//! flower releases on a tag: pushing `vX.Y.Z` starts `publish.yml`, which
//! uploads to crates.io. Everything before that push is mechanical and easy to
//! get half-right by hand — the workspace version lives in more than one place
//! in the root manifest, the lockfile has to follow it, and the changelog's
...
const USER_AGENT: &str = "flower-release (xtask; https://github.com/diaryx-org/flower)";

// ---------------------------------------------------------------------------
// Versions
// ---------------------------------------------------------------------------

/// A semver triple, which is all flower has ever used. Pre-release and build
...
/// `flower-core = { path = "crates/flower-core", version = "0.1.0" }` with the
...
///
/// A tag can appear after the fact — cut once the release it stood for had
/// already gone to crates.io — and without this, those commits would simply
/// leave the file: no longer unreleased, and in no section either. So the tag
/// list, not the last write, decides what the changelog owes.
...
/// `## v0.2.0 — 2026-08-18` -> `v0.2.0`, and anything else -> `None`.
...
pub fn release_notes(sh: &Sh, tag: Option<&str>) -> Result<()> {
    ...
    println!(
        "\n---\n\n\
         ...
         [docs/CHANGELOG.md](https://github.com/diaryx-org/flower/blob/{tag}/docs/CHANGELOG.md)."
    );
    Ok(())
}
...
pub fn members(sh: &Sh) -> Result<Vec<Member>> {
    let manifest = sh.read("Cargo.toml")?;
    let start = manifest
        .find("members")
...
fn dependencies_on_members(manifest: &str, names: &[String]) -> Vec<String> {
    let is_member = |name: &str| names.iter().any(|m| m == name);
    ...
    for line in manifest.lines() {
        ...
        if let Some(header) = line.strip_prefix('[').and_then(|l| l.strip_suffix(']')) {
            ...
            // `[dependencies.flower-core]` — the dependency is named by the
        ...
        // `flower-core = { workspace = true }` and `flower-core.workspace = true`
...
/// Every publishable member, by package name, ordered so that no crate is
/// published before something it depends on. crates.io enforces this — an upload
/// whose dependencies are not yet on the index is rejected — and the order is
/// derived rather than written down so that adding a crate to the workspace is
/// enough.
fn publish_order(sh: &Sh) -> Result<Vec<String>> {
    ...
    for member in &members {
        let text = sh.read(&format!("{}/Cargo.toml", member.path))?;
        let deps = dependencies_on_members(&text, &names);
        manifests.push((member.name.clone(), member.publishable, deps));
    }
    ...
    for name in &names {
        visit(name, &manifests, &mut order, &mut visiting)?;
    }
    Ok(order)
}
...
/// starts is worth stating plainly: `publish.yml` uploads every publishable
/// crate to crates.io, and none of it is reversible; a yanked crates.io version
/// is still a spent version number.
pub fn release(sh: &Sh, spec: &str, args: &[&str]) -> Result<()> {
    ...
    // Annotated: the release workflow reads `github.ref_name`, and `git
    // describe` wants an object to read.
    ...
    if !push {
        println!(
            "\n\x1b[32m{tag} is committed and tagged locally.\x1b[0m\n\n\
             ...
             git push origin {tag}\n\n\
             The tag is what publishes: `publish.yml` uploads {} crate(s) to crates.io,\n\
    ...
    println!(
        "\n\x1b[32m{tag} pushed.\x1b[0m Publish is running:\n    \
         https://github.com/diaryx-org/flower/actions\n"
    );
    Ok(())
}
...
fn preflight(sh: &Sh, next: Version, tag: &str) -> Result<()> {
    ...
    if branch.trim() != RELEASE_BRANCH {
        return Err(format!(
            "on branch `{}`, and flower releases from `{RELEASE_BRANCH}`",
            branch.trim()
        ));
    }
    ...
    // The tag is not the only way a version gets spent — a crate can go up from
    // a laptop, untagged — so ask the registry rather than the tag list.
    ...
    // A release cut on a stale branch is a release missing commits. Fetch is
...
mod tests {
    ...
    fn path_dependencies_follow_the_workspace_version() {
        ...
        assert_eq!(
            retarget_path_dependency(
                r#"flower-core = { path = "crates/flower-core", version = "0.1.0" }"#,
                next
            )
            .unwrap(),
            r#"flower-core = { path = "crates/flower-core", version = "0.2.0" }"#
        );
        // A path dependency with no version requirement is left alone — an
        // unpublished sibling has nothing for cargo to resolve from the index.
        assert_eq!(
            retarget_path_dependency(
                r#"flower-ratatui = { path = "crates/flower-ratatui" }"#,
                next
            ),
            None
        );
        ...
        assert_eq!(
            retarget_path_dependency(r#"fig = { version = "3" }"#, next),
            None
        );
        assert_eq!(retarget_path_dependency(r#"version = "0.1.0""#, next), None);
    }
    ...
    /// The real workspace: every member found, named, and read.
    #[test]
    ...
    fn member_dependencies_are_found_in_every_spelling() {
        let names: Vec<String> = ["flower-core", "flower-ratatui", "flower-tui"]
            .iter()
            .map(|s| s.to_string())
            .collect();
        let manifest = r#"
...
[dev-dependencies.flower-tui]
path = "."
"#;
        assert_eq!(
            dependencies_on_members(manifest, &names),
            vec!["flower-core", "flower-ratatui", "flower-tui"]
        );
    ...
    /// and everything marked `publish = false` left out.
    #[test]
    fn publish_order_respects_the_workspace() {
        let sh = Sh::new();
        let order = publish_order(&sh).unwrap();
        let found = members(&sh).unwrap();
        let names: Vec<String> = found.iter().map(|m| m.name.clone()).collect();

        for member in &found {
            assert_eq!(
                order.contains(&member.name),
                member.publishable,
                "`{}` is published: {}, but publish = false is {}",
                member.name,
                order.contains(&member.name),
                !member.publishable,
            );
        ...
        for (position, name) in order.iter().enumerate() {
            let path = &found.iter().find(|m| &m.name == name).unwrap().path;
            let manifest = sh.read(&format!("{path}/Cargo.toml")).unwrap();
            for dep in dependencies_on_members(&manifest, &names) {
                if dep == *name {
                    continue;
                }
    ...
    fn rewriting_leaves_everything_outside_the_region_alone() {
        ...
        let cut = rewrite(&text, EMPTY_REGION, Some("## v0.2.0 — 2026-08-18\n\nnew\n")).unwrap();
    ...
    fn tag_headings_are_told_from_prose_ones() {
        assert_eq!(section_tag("## v0.2.0 — 2026-08-18"), Some("v0.2.0"));
        assert_eq!(section_tag("## v0.2.0"), Some("v0.2.0"));
        assert_eq!(section_tag("## Unreleased"), None);
        assert_eq!(section_tag("## verify the build"), None);
        assert_eq!(section_tag("### Added"), None);
        assert_eq!(section_tag("- **tui** — ## v1.0.0 in a bullet"), None);
    }
